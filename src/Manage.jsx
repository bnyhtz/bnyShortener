...
import './App.css';
import CreateEmbedded from './CreateEmbedded';
...
export default function Manage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const s2 = await fetch('/api/auth/session', { cache: 'no-store' });
        const sj = await s2.json();
  ...
  const doEdit = (link) => setEditing(link);

  const saveEdit = async ({ path, newPath, domain, url }) => {
    setSavingEdit(true);
    let headers = { 'Content-Type': 'application/json' };
    if (passwordProtected && !sessionAuthenticated) {
  ...
  const handleLogin = async (username, password, remember) => {
    ...
    try {
      ...
      if (resp.ok && js.verified) {
        ...
        try {
          if (remember) window.localStorage.setItem('rememberedUsername', username || ''); else window.localStorage.removeItem('rememberedUsername');
        } catch (e) {}
      ...
      try { notify('Login failed', { duration: 3000 }); } catch (er) { dlgAlert('Login failed'); }
  ...
  return (
    <div className="container manage-page dash-layout">
      <div className="left">
        <CreateEmbedded onCreated={() => fetchLinks(filterDomain || null)} />
      </div>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
              {passwordProtected && !sessionAuthenticated && (
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                  ...
                  <button className="primary" onClick={() => handleLogin(loginUsername, loginPassword, loginRemember)} disabled={loginLoading}>{loginLoading ? 'Logging in...' : 'Login'}</button>
                </div>
              )}

              {sessionAuthenticated && (
                <>
                  <button className="primary" onClick={() => { try { window.location.pathname = '/dash'; } catch (e) {} }}>Create link</button>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                ...
                {filtered.map(l => (
                  <tr key={l.path} style={{ borderTop: '1px solid rgba(255,255,255,0.04)' }}>
                    <td style={{ padding: '0.75rem' }}><a href={l.shortUrl} target="_blank" rel="noreferrer">{l.shortUrl}</a></td>
                    <td style={{ padding: '0.75rem' }}><a href={l.originalUrl} target="_blank" rel="noreferrer">{l.originalUrl}</a></td>
                    ...
                    <td style={{ padding: '0.75rem' }}>
                      <button className="primary" onClick={async () => { try { await navigator.clipboard.writeText(l.shortUrl); try { notify('Copied', { duration: 1500 }); } catch (er) { dlgAlert('Copied'); } } catch { try { notify('Copy failed', { duration: 3000 }); } catch (er) { dlgAlert('Copy failed'); } } }}>Copy</button>
                      <button className="primary" style={{ marginLeft: '0.5rem' }} onClick={() => window.open(l.shortUrl, '_blank')}>Open</button>
                      <button className="primary" style={{ marginLeft: '0.5rem' }} onClick={() => doEdit(l)}>Edit</button>
                      <button className="primary" style={{ marginLeft: '0.5rem' }} onClick={() => doDelete(l.path)}>Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
